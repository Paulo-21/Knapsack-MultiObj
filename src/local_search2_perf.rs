...
fn get_voisins(
    ...
    pop_aux: &mut Vec<(Vec<bool>, Vec<u32>)>,
) {
    let (take, profit) = x;
    let p = profit.clone();
            ...
            for s in weight_sorted_idx {
                if !take[*s] && tot_weight + w[*s] <= max_cap && *s != k {
                    ...
                    if !(p[0] >= profit[0] && p[1] >= profit[1]) {
                        ...
                        let mut k =
                            match pareto_front.binary_search_by(|pt| pt.1[0].cmp(&profit[0])) {
                                Ok(j) => j,
                                Err(index) => index,
                            };

                        if k < pareto_front.len() {
                            if profit[0] == pareto_front[k].1[0] {
                                if profit[1] > pareto_front[k].1[1] {
                                    //Changer si meilleur que pareil
                                    pareto_front[k] = (take.clone(), profit.clone());
                                } else {
                                    updated = false;
                                }
                            } else if profit[0] < pareto_front[k].1[0] {
                                if profit[1] > pareto_front[k].1[1] {
